...
Parser::Parser(std::vector<Token> parserTokens, Diagnostics &diagnostics,
               std::string fname)
    : tokens(parserTokens), filename(fname), diags(diagnostics) {}
...
Token Parser::expect(TokenType ttype) {
  ...
  diags.report<SyntaxError>("Expected " + tokenTypeToString(ttype) + ", got " +
                                tokenTypeToString(current().kind),
    ...
    try {
      ...
      if (!stmt.noOp) {
        if (auto imported = dynamic_cast<Program *>(stmt.expr.get())) {
          std::vector<ExpressionStmt> imported_stmts =
              std::move(imported->statements);
...
std::vector<ExpressionStmt> Parser::parse_block() {
  ...
  while (current().kind != TokenType::CLOSE_BRAC) {
    if (current().kind == TokenType::EOF_TOK) {
      diags.report<SyntaxError>("Closing braces required for code block",
                                current().span,
...
ExpressionStmt Parser::parse_statement() {
  ...
  if (token.kind == TokenType::LOAD) {
    ...
    ASTPtr loadStmt = std::make_unique<LoadStmt>(
        fname, Span::combine(token.span, current().span));

    return ExpressionStmt(std::move(loadStmt),
                          Span::combine(token.span, current().span), false,
                          false, false);
  } else if (token.kind == TokenType::SEM) {
    advance();

    ExpressionStmt expressionStmt = ExpressionStmt(
        std::make_unique<NoOp>(), current().span, true, false, false);
    ...
    while (current().kind != TokenType::CLOSE_PAREN) {
      if (current().kind == TokenType::EOF_TOK) {
        diags.report<SyntaxError>(
            ...
            current().span, "Did you forget closing parenthesis?", filename);
      ...
      params.push_back(
          std::make_unique<Variable>(param.text, current().span, nullptr));
    ...
    if (token.kind == TokenType::FORM) {
      res = std::make_unique<FunctionStmt>(
          name.text, std::move(params), std::move(stmts),
          Span::combine(token.span, parenSpan), nullptr);
    } else {
      res = std::make_unique<ClassStmt>(name.text, std::move(params),
                                        std::move(stmts),
                                        Span::combine(token.span, parenSpan));
    }

    return ExpressionStmt(std::move(res), Span::combine(token.span, parenSpan),
                          false, false, false);
    ...
    ASTPtr returnStmt = std::make_unique<ReturnStmt>(
        std::move(value), Span::combine(token.span, current().span));
    ...
    ASTPtr whileStmt =
        std::make_unique<WhileStmt>(std::move(condition), std::move(stmts),
                                    Span::combine(token.span, endSpan));

    return ExpressionStmt(std::move(whileStmt),
                          Span::combine(token.span, endSpan), false, false,
                          false);
    ...
    if (!forVar) {
      diags.report<SyntaxError>(
          "Expected a variable name as the for-loop iteration variable",
          token.span, "", filename);
      exitErrors();
    }
    ASTPtr forStmt = std::make_unique<ForStmt>(
        forVar->name, std::move(iter), std::move(stmts),
        Span::combine(token.span, endSpan));
    ...
    if (current().kind != TokenType::ELSE) {
      ASTPtr ifStmt = std::make_unique<IfStmt>(
          std::move(condition), std::move(thenStmts),
          Span::combine(token.span, endSpan), std::vector<ExpressionStmt>());
    ...
    ASTPtr ifStmt = std::make_unique<IfStmt>(
        std::move(condition), std::move(thenStmts),
        Span::combine(token.span, endSpan), std::move(elseStmts));

    return ExpressionStmt(std::move(ifStmt), Span::combine(token.span, endSpan),
                          false, false, false);
  } else if (token.kind == TokenType::BREAK ||
    ...
    return ExpressionStmt(std::make_unique<NoOp>(),
                          Span::combine(token.span, current().span), true, true,
                          token.kind == TokenType::CONTINUE);
  }
  ...
  return ExpressionStmt(std::move(expr),
                        Span::combine(token.span, current().span), false, false,
                        false);
}
...
ASTPtr Parser::parse_expression(int min_bp) {
  ...
  if (token.kind == TokenType::BIT_NOT || token.kind == TokenType::NOT ||
      token.kind == TokenType::SUB || token.kind == TokenType::INCREMENT ||
      token.kind == TokenType::DECREMENT) {
    advance();

    ASTPtr operand = parse_expression(15);

    TokenType new_token_type =
        (token.kind == TokenType::SUB) ? TokenType::NEGATE : token.kind;

    left = std::make_unique<UnaryOp>(new_token_type, std::move(operand),
                                     Span::combine(token.span, current().span));
  } else if (token.kind >= TokenType::INT_HEX &&
             token.kind <= TokenType::INT_BIN) {
    int8_t base = -1;
    ...
    if (token.kind == TokenType::INT_BIN)
      base = 2;
    if (base == -1) {
      diags.report<SyntaxError>("Invalid radix base", current().span,
                                "Please report this in the Issue Tracker",
                                filename);
    }

    left = std::make_unique<IntLiteral>(
        std::strtoll(token.text.c_str(), NULL, base), current().span);
    ...
    left =
        std::make_unique<StrLiteral>(read_escape(token.text), current().span);
    ...
    while (current().kind != TokenType::CLOSE_BRACKET) {
      if (current().kind == TokenType::EOF_TOK) {
        diags.report<SyntaxError>("Unterminated vector literal", current().span,
    ...
    left = std::make_unique<VecLiteral>(
        std::move(elems), Span::combine(token.span, current().span));
    ...
    while (current().kind != TokenType::CLOSE_BRAC) {
      if (current().kind == TokenType::EOF_TOK) {
        diags.report<SyntaxError>("Unterminated dictionary literal",
                                  current().span,
                                  "Did you forget a closing brace?", filename);
    ...
    left = std::make_unique<DicLiteral>(
        std::move(dic), Span::combine(token.span, current().span));
    ...
    if (peek().kind == TokenType::OPEN_PAREN) {
      ...
      ASTPtr call = std::make_unique<FunctionCall>(
          token.text, std::move(params),
          Span::combine(token.span, current().span));

      left = std::move(call);
    } else {
      left = std::make_unique<Variable>(
          token.text, Span::combine(token.span, current().span), nullptr);
    }
  ...
  while (true) {
    ...
    if (nextToken.kind == TokenType::INCREMENT ||
        nextToken.kind == TokenType::DECREMENT) {
      advance();

      left =
          std::make_unique<UnaryOp>(nextToken.kind, std::move(left),
                                    Span::combine(startSpan, nextToken.span));
      continue;
    }
    ...
    left =
        std::make_unique<BinaryOp>(op.kind, std::move(left), std::move(right),
                                   Span::combine(startSpan, current().span));
  }
